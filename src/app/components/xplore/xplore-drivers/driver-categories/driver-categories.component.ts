...
export class DriverCategoriesComponent implements OnInit {
  ...
  loadData(){
    ...
    const usersSubscription = this.usersService.getDriverCategories(this.currentDriver)
      .subscribe(response => {
  ...
  removeDriverCategory(category){
    this.loaders.loadingData = true
    const usrsSubs = this.usersService.removeDriverCategory(this.currentDriver, category)
      .subscribe(response => {
