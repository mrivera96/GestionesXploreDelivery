...
export class AddWorkLineComponent implements OnInit {
  ...
  loadData() {
    ...
    const wLSubscription = this.workLinesService.getWorkLines()
      .subscribe(response => {
  ...
  onFormSubmit() {
    if (this.wlForm.valid) {
      ...
      const wlsubscription = this.workLinesService.addCustomerToWorkLine(this.currentCustomer, this.wlForm.get('idRubro').value)
        .subscribe(response => {
