...
export class PaymentsReportComponent implements OnInit {
  ...
  loadData() {
    ...
    const paymentsSubscription = this.paymentsService
      .getPaymentsReport(this.consultForm.value).subscribe(response => {
  ...
  generateExcel() {
    ...
    this.consultResults.forEach(d => {
      let array = [index, d.fechaPago, d.monto, d.payment_type.nomTipoPago, d.customer.nomEmpresa,
        d.referencia, d.banco, d.numAutorizacion]
      arrayRow.push(array)
      index++
    })
    arrayRow.forEach(v => {
    ...
    const paymentstotals = worksheet.addRow(['', 'Total:', this.totalAmount, '', '', '', '', '']);
