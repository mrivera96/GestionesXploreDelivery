...
export class NewWorklineDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.newWLForm = new FormGroup(
      {
        nomRubro: new FormControl('', [Validators.required,Validators.maxLength(60)]),
        descRubro: new FormControl('',[Validators.required, Validators.maxLength(250)])
      }
    )
  }
  ...
  onFormNewSubmit() {
    if (this.newWLForm.valid) {
      ...
      const worklinesSubscription = this.worklinesService.createWorkLine(this.newWLForm.value)
        .subscribe(response => {
