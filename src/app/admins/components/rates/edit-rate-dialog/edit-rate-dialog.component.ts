...
export class EditRateDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.edRateForm = this.formBuilder.group({
      ...
      cobVehiculo: [
        this.currRate?.item_detail?.cobVehiculo || 0,
        Validators.min(0),
      ],
      servChofer: [
        this.currRate?.item_detail?.servChofer || 0,
        Validators.min(0),
      ],
      recCombustible: [
        this.currRate?.item_detail?.recCombustible || 0,
        Validators.min(0),
      ],
      cobTransporte: [
        this.currRate?.item_detail?.cobTransporte || 0,
        Validators.min(0),
      ],
      isv: [this.currRate?.item_detail?.isv || 0, Validators.min(0)],
      tasaTuris: [
        this.currRate?.item_detail?.tasaTuris || 0,
        Validators.min(0),
      ],
      gastosReembolsables: [
        this.currRate?.item_detail?.gastosReembolsables || 0,
        Validators.min(0),
      ],
      transportePersonas: [
        this.currRate?.item_detail?.transportePersonas || 0,
        Validators.min(0),
      ],
    });

    const cateoriesSubscription = this.categoriesService
      .getAllCategories()
      .subscribe((response) => {
    ...
    if (this.edRateForm.valid) {
      ...
      if (
        ...
        if (sum > toCompare || sum < toCompare) {
          ...
          const rateSubsc = this.ratesService
            .editRate(this.edRateForm.value)
            .subscribe(
              (response) => {
                this.loaders.loadingSubmit = false;
