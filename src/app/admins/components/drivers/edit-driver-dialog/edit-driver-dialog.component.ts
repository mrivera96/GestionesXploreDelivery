...
export class EditDriverDialogComponent implements OnInit {
  ...
  initialize() {
    this.edDrivForm = this.formBuilder.group({
      ...
      nomUsuario: [
        this.currDriver.nomUsuario,
        [Validators.required, Validators.maxLength(80)],
      ],
      nickUsuario: [
        this.currDriver.nickUsuario,
        [Validators.required, Validators.maxLength(40)],
      ],
      passcodeUsuario: [
        this.currDriver.passcodeUsuario,
        [Validators.required, Validators.minLength(4), Validators.maxLength(4)],
      ],
      numCelular: [
        this.currDriver.numCelular,
  ...
  onEditFormSubmit() {
    if (this.edDrivForm.valid) {
      ...
      this.usersService.editDriver(this.edDrivForm.value).subscribe(
        (response) => {
