...
export class NewDriverDialogComponent implements OnInit {
  ...
  initialize() {
    this.nDrivForm = this.formBuilder.group({
      nomUsuario: ['', [Validators.required, Validators.maxLength(80)]],
      nickUsuario: ['', [Validators.required, Validators.maxLength(40)]],
      passcodeUsuario: [
        '',
  ...
  onNewFormSubmit() {
    if (this.nDrivForm.valid) {
      ...
      this.usersService.addDriver(this.nDrivForm.value).subscribe(
        (response) => {
