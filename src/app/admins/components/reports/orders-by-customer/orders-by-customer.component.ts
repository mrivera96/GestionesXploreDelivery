...
export class OrdersByCustomerComponent implements OnInit {
  ...
  onConsultFormSubmit() {
    if (this.consultForm.valid) {
      ...
      this.totalSurcharges = 0.0;
      this.totalCosts = 0.0;
      this.deliveriesService
        .getOrdersByCustomer(this.consultForm.value)
        .subscribe((response) => {
  ...
  generateExcel() {
    ...
    //Blank Row
    worksheet.addRow([]);
    let totalOrders = worksheet.addRow([
      'Envíos Totales : ' + this.totalCustomerOrders,
    ]);
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(
        blob,
        'Reporte envíos (' + this.currenCustomer.nomEmpresa + ').xlsx'
      );
    });
  }
  ...
  generatePDF() {
    ...
    pdf.add(new Columns(ordersByCategoryHeader).bold().end);
    arrayRow.forEach((res) => {
      pdf.add(new Columns(res).end);
    });
    ...
    pdf.add(new Columns(ordersCategoriestotals).bold().end);
    ...
    pdf.add(new Columns(ordersByDateHeader).bold().end);
    ...
    pdf.add(new Columns(ordersRange).bold().end);
    ...
    pdf.add(new Txt(detailTitle).bold().end);
    pdf.add(pdf.ln(2));
