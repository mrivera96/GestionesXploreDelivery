...
export class CustomersBalanceReportComponent implements OnInit {
  ...
  onConsultFormSubmit() {
    if (this.consultForm.valid) {
      ...
      const reportSubscription = this.usersService
        .getCustomersBalanceReport(this.consultForm.value)
        .subscribe((response) => {
  ...
  generatePDF() {
    ...
    pdf.add(new Columns(paymentsHeader).bold().end);

    arrayRow.forEach((v) => {
      pdf.add(new Columns(v).end);
    });
    ...
    const paymentstotals = [
      ...
      'Total:',
      this.totalOrders,
      'L. ' + this.totalPayments,
      'L. ' + this.totalBalance,
      'L. ' + this.totalCredit,
    ];
    pdf.add(new Columns(paymentstotals).bold().end);
