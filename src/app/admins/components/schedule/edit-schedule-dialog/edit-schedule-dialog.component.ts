...
export class EditScheduleDialogComponent implements OnInit {
  ...
  initialize() {
    this.editSchForm = this.formBuilder.group({
      ...
      inicio: [
        formatDate('2020-06-29 ' + this.currentSchedule?.inicio, 'HH:mm', 'en'),
        [Validators.required],
      ],
      final: [
        formatDate('2020-06-29 ' + this.currentSchedule?.final, 'HH:mm', 'en'),
        [Validators.required],
      ],
    });
  }
  ...
  onEditFormSubmit() {
    if (this.editSchForm.valid) {
      ...
      this.schedulesService.editSchedule(this.editSchForm.value).subscribe(
        (response) => {
