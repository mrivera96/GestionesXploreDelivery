...
export class EditExtraChargeOptionDialogComponent implements OnInit {
  ...
  initialize() {
    this.exChrgOptForm = this.formBuilder.group({
      ...
      gastosReembolsables: [this.currOption?.item_detail?.gastosReembolsables || 0],
      transportePersonas: [this.currOption?.item_detail?.transportePersonas || 0]
    })
  }
    ...
    if (this.exChrgOptForm.valid) {
      ...
      if (tk != 0 || cobVeh != 0 || servChof != 0 || recComb != 0
        ...
        if (sum != this.currOption.costo) {
          this.openErrorDialog('Los valores ingresados para facturación, no coinciden con el monto del cargo extra')
